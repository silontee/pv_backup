"""Step 2 — ResMLP+AdaLN with learnable site embedding (one-hot replace).

기존 ResMLP+AdaLN: site one-hot (8 dims, sparse) → cond에 직접 concat
새 버전:           site embedding (Embedding(8, 16)) → cond에 concat

다른 부분 모두 동일 (architecture, hyperparams, training).

3 seeds (42, 123, 7) → Ensemble baseline 대비 비교.
초점: 광양항세방 / 예천 / 고흥만수상 NMAE + Cov80 변화.
"""
import os
import random
import sys
import warnings
from pathlib import Path

import numpy as np
import pandas as pd
import torch
import torch.nn as nn
import torch.nn.functional as F
from torch.utils.data import DataLoader, TensorDataset

if hasattr(sys.stdout, "reconfigure"):
    sys.stdout.reconfigure(encoding="utf-8")
warnings.filterwarnings("ignore")

ROOT = Path(__file__).resolve().parents[2]
sys.path.insert(0, str(ROOT / "src/models"))

from train_resmlp_adaln import (
    AdaLNBlock, MAIN_FEATURES, COND_NUM_FEATURES,
    TARGET, TRAIN_END, VAL_END, DIM, N_BLOCKS, LR, BATCH_SIZE,
    MAX_EPOCHS, PATIENCE, gaussian_nll,
)

SITE_EMB_DIM = 16
SEEDS = [42, 123, 7]
OUT_DIR = ROOT / "pv/experiments/resmlp_adaln_site_emb"
OUT_DIR.mkdir(parents=True, exist_ok=True)


def set_all_seeds(seed):
    random.seed(seed); np.random.seed(seed)
    torch.manual_seed(seed); torch.cuda.manual_seed_all(seed)
    os.environ["PYTHONHASHSEED"] = str(seed)
    torch.backends.cudnn.deterministic = True
    torch.backends.cudnn.benchmark = False


def load_data():
    df = pd.read_parquet(ROOT / "data/processed/training_set.parquet")
    df = df[df.dsr_mean.notna() & df.cf.notna()].copy()
    for col in ["ta", "hm", "ws", "dc10Tca"]:
        df[col] = df.groupby("site")[col].transform(lambda s: s.fillna(s.median()))
    df["rn"] = df["rn"].fillna(0)
    return df


def build(df, all_sites):
    main = df[MAIN_FEATURES].values.astype(np.float32)
    cond_num = df[COND_NUM_FEATURES].values.astype(np.float32)
    site_idx = {s: i for i, s in enumerate(all_sites)}
    site_id = df["site"].map(site_idx).values.astype(np.int64)
    y = df[TARGET].values.astype(np.float32)
    return main, cond_num, site_id, y


class ResMLPAdaLN_SiteEmb(nn.Module):
    def __init__(self, n_main, n_cond_num, n_sites, site_emb_dim=SITE_EMB_DIM,
                 dim=DIM, n_blocks=N_BLOCKS):
        super().__init__()
        self.site_emb = nn.Embedding(n_sites, site_emb_dim)
        n_cond = n_cond_num + site_emb_dim   # cond = num features + site embedding

        self.input_proj = nn.Linear(n_main, dim)
        self.blocks = nn.ModuleList([AdaLNBlock(dim, n_cond) for _ in range(n_blocks)])
        self.final_norm = nn.LayerNorm(dim, elementwise_affine=False)
        self.cond_final = nn.Linear(n_cond, 2 * dim)
        self.mu_head = nn.Linear(dim, 1)
        self.log_sig_head = nn.Linear(dim, 1)

    def forward(self, x_main, x_cond_num, site_id):
        s_e = self.site_emb(site_id)
        x_cond = torch.cat([x_cond_num, s_e], dim=-1)
        x = self.input_proj(x_main)
        for blk in self.blocks:
            x = blk(x, x_cond)
        h = self.final_norm(x)
        scale, shift = self.cond_final(x_cond).chunk(2, dim=-1)
        h = h * (1 + scale) + shift
        mu = self.mu_head(h).squeeze(-1)
        sigma = F.softplus(self.log_sig_head(h)).squeeze(-1) + 1e-3
        return mu, sigma


def predict(model, X_main, X_cond_num, site_id, Y, device):
    model.eval()
    ds = TensorDataset(torch.from_numpy(X_main), torch.from_numpy(X_cond_num),
                       torch.from_numpy(site_id), torch.from_numpy(Y))
    loader = DataLoader(ds, batch_size=BATCH_SIZE * 2)
    mus, sigs = [], []
    with torch.no_grad():
        for xm, xc, sid, _ in loader:
            mu, sig = model(xm.to(device), xc.to(device), sid.to(device))
            mus.append(mu.cpu().numpy()); sigs.append(sig.cpu().numpy())
    return np.clip(np.concatenate(mus), 0, None), np.concatenate(sigs)


def eval_metrics(y, mu, sigma, cap):
    err = np.abs(y - mu)
    nmae = (err * cap).sum() / cap.sum() * 100
    bias = ((mu - y) * cap).sum() / cap.sum() * 100
    cov80 = ((y >= mu - 1.282 * sigma) & (y <= mu + 1.282 * sigma)).mean() * 100
    cov95 = ((y >= mu - 1.96 * sigma) & (y <= mu + 1.96 * sigma)).mean() * 100
    nll = (0.5 * np.log(2 * np.pi * sigma ** 2) + (y - mu) ** 2 / (2 * sigma ** 2)).mean()
    return {"nmae": nmae, "bias": bias, "cov80": cov80, "cov95": cov95, "nll": float(nll)}


def per_site_metrics(y, mu, sigma, sites, cap):
    df = pd.DataFrame({"site": sites, "y": y, "mu": mu, "sigma": sigma, "cap": cap})
    df["err"] = (df.y - df.mu).abs()
    df["in_80"] = ((df.y >= df.mu - 1.282 * df.sigma) & (df.y <= df.mu + 1.282 * df.sigma)).astype(int)
    df["in_95"] = ((df.y >= df.mu - 1.96 * df.sigma) & (df.y <= df.mu + 1.96 * df.sigma)).astype(int)
    out = df.groupby("site").apply(lambda g: pd.Series({
        "n": len(g),
        "nmae": (g.err * g.cap).sum() / g.cap.sum() * 100,
        "bias": ((g.mu - g.y) * g.cap).sum() / g.cap.sum() * 100,
        "cov80": g.in_80.mean() * 100,
        "cov95": g.in_95.mean() * 100,
    }), include_groups=False).reset_index()
    return out


def train_one(seed, df, all_sites, device):
    print(f"\n{'='*70}")
    print(f"[Seed {seed}] (Site embedding)")
    print(f"{'='*70}")
    set_all_seeds(seed)
    train_df = df[df.datetime_kst < TRAIN_END]
    val_df = df[(df.datetime_kst >= TRAIN_END) & (df.datetime_kst < VAL_END)]
    test_df = df[df.datetime_kst >= VAL_END]

    Xm_tr, Xcn_tr, Sid_tr, Ytr = build(train_df, all_sites)
    Xm_v, Xcn_v, Sid_v, Yv = build(val_df, all_sites)
    Xm_te, Xcn_te, Sid_te, Yte = build(test_df, all_sites)

    n_main = Xm_tr.shape[1]; n_cond_num = Xcn_tr.shape[1]
    model = ResMLPAdaLN_SiteEmb(n_main, n_cond_num, len(all_sites)).to(device)
    n_params = sum(p.numel() for p in model.parameters())
    print(f"  params: {n_params/1e3:.1f}k")

    train_ds = TensorDataset(torch.from_numpy(Xm_tr), torch.from_numpy(Xcn_tr),
                              torch.from_numpy(Sid_tr), torch.from_numpy(Ytr))
    val_ds = TensorDataset(torch.from_numpy(Xm_v), torch.from_numpy(Xcn_v),
                            torch.from_numpy(Sid_v), torch.from_numpy(Yv))
    g = torch.Generator(); g.manual_seed(seed)
    train_loader = DataLoader(train_ds, batch_size=BATCH_SIZE, shuffle=True, generator=g)
    val_loader = DataLoader(val_ds, batch_size=BATCH_SIZE)

    opt = torch.optim.AdamW(model.parameters(), lr=LR, weight_decay=1e-4)
    sch = torch.optim.lr_scheduler.CosineAnnealingLR(opt, T_max=MAX_EPOCHS)
    best = float("inf"); best_state = None; bad = 0
    for ep in range(MAX_EPOCHS):
        model.train()
        for xm, xc, sid, y in train_loader:
            xm, xc, sid, y = xm.to(device), xc.to(device), sid.to(device), y.to(device)
            mu, sig = model(xm, xc, sid)
            loss = gaussian_nll(y, mu, sig).mean()
            opt.zero_grad(); loss.backward()
            torch.nn.utils.clip_grad_norm_(model.parameters(), 1.0)
            opt.step()
        sch.step()
        model.eval()
        v = 0; nv = 0
        with torch.no_grad():
            for xm, xc, sid, y in val_loader:
                xm, xc, sid, y = xm.to(device), xc.to(device), sid.to(device), y.to(device)
                mu, sig = model(xm, xc, sid)
                v += gaussian_nll(y, mu, sig).mean().item() * len(y); nv += len(y)
        v /= nv
        if v < best:
            best = v; bad = 0
            best_state = {k: w.cpu().clone() for k, w in model.state_dict().items()}
        else:
            bad += 1
        if ep % 20 == 0:
            print(f"  ep {ep:>3}: val_loss {v:.4f}", flush=True)
        if bad >= PATIENCE:
            print(f"  early stop @ ep {ep}", flush=True); break
    model.load_state_dict(best_state)

    mu_v, sig_v = predict(model, Xm_v, Xcn_v, Sid_v, Yv, device)
    mu_te, sig_te = predict(model, Xm_te, Xcn_te, Sid_te, Yte, device)

    cap_te = test_df["site_capacity_kw"].values
    sites_te = test_df["site"].values
    m = eval_metrics(Yte, mu_te, sig_te, cap_te)
    print(f"  Test: NMAE {m['nmae']:.3f}% / Cov80 {m['cov80']:.1f}% / NLL {m['nll']:.4f}",
          flush=True)

    out = OUT_DIR / f"seed_{seed}"
    out.mkdir(parents=True, exist_ok=True)
    pd.DataFrame({
        "datetime_kst": test_df["datetime_kst"].values, "site": sites_te,
        "site_capacity_kw": cap_te, "cf": Yte, "pred_mu": mu_te, "pred_sigma": sig_te,
    }).to_parquet(out / "test_predictions.parquet", index=False)
    return m, mu_te, sig_te, sites_te, cap_te, Yte


def ensemble_from_seeds():
    dfs = []
    for seed in SEEDS:
        p = OUT_DIR / f"seed_{seed}/test_predictions.parquet"
        df = pd.read_parquet(p); df["seed"] = seed
        dfs.append(df)
    all_df = pd.concat(dfs, ignore_index=True)
    grp = all_df.groupby(["datetime_kst", "site"])
    ens = grp.agg(
        cf=("cf", "first"),
        site_capacity_kw=("site_capacity_kw", "first"),
        mu_mean=("pred_mu", "mean"),
        mu_var=("pred_mu", "var"),
        sigma_sq_mean=("pred_sigma", lambda s: (s ** 2).mean()),
    ).reset_index()
    ens["sigma_total"] = np.sqrt(ens["sigma_sq_mean"] + ens["mu_var"])
    return ens


def main():
    print("=" * 70)
    print(f"Step 2 — ResMLP+AdaLN + site embedding (seeds: {SEEDS})")
    print("=" * 70)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    df = load_data()
    all_sites = sorted(df["site"].unique())
    print(f"\n  Sites ({len(all_sites)}): {all_sites}")
    print(f"  Site embedding dim: {SITE_EMB_DIM} (one-hot 8 → embedding 16)")

    per_seed = []
    for seed in SEEDS:
        m, _, _, _, _, _ = train_one(seed, df, all_sites, device)
        m["seed"] = seed
        per_seed.append(m)

    seed_df = pd.DataFrame(per_seed)
    print("\n" + "=" * 70)
    print("Per-seed (test, site embedding)")
    print("=" * 70)
    print(seed_df.round(3).to_string(index=False))
    print(f"\n  Mean ± Std:")
    for col in ["nmae", "bias", "cov80", "cov95", "nll"]:
        v = seed_df[col].values
        print(f"    {col:<10} {v.mean():>7.3f} ± {v.std(ddof=1):>5.3f}")

    # Ensemble
    print("\n" + "=" * 70)
    print("Ensemble (3 seeds, site embedding)")
    print("=" * 70)
    ens = ensemble_from_seeds()
    cap = ens["site_capacity_kw"].values
    y = ens["cf"].values
    mu = ens["mu_mean"].values
    sigma = ens["sigma_total"].values
    m_ens = eval_metrics(y, mu, sigma, cap)
    print(f"  Ensemble: NMAE {m_ens['nmae']:.3f}% / bias {m_ens['bias']:+.3f}% / "
          f"Cov80 {m_ens['cov80']:.1f}% / Cov95 {m_ens['cov95']:.1f}% / NLL {m_ens['nll']:.4f}")

    print("\n  Per-site (ensemble, site embedding):")
    site_ens = per_site_metrics(y, mu, sigma, ens["site"].values, cap)
    print(site_ens.round(2).to_string(index=False))

    # Comparison
    print("\n" + "=" * 70)
    print("vs Step 1 ensemble baseline")
    print("=" * 70)
    print(f"  {'Metric':<20} {'Baseline (5 seeds)':<22} {'Site Emb (3 seeds)':<22} {'Δ':>10}")
    base_nmae = 5.424; base_bias = -0.518; base_cov80 = 86.5; base_cov95 = 94.2; base_nll = -1.174
    print(f"  {'NMAE':<20} {base_nmae:>15.3f}%       {m_ens['nmae']:>15.3f}%       "
          f"{m_ens['nmae']-base_nmae:>+9.3f}")
    print(f"  {'Bias':<20} {base_bias:>15.3f}%       {m_ens['bias']:>15.3f}%       "
          f"{m_ens['bias']-base_bias:>+9.3f}")
    print(f"  {'Cov80':<20} {base_cov80:>15.1f}%       {m_ens['cov80']:>15.1f}%       "
          f"{m_ens['cov80']-base_cov80:>+9.2f}")
    print(f"  {'Cov95':<20} {base_cov95:>15.1f}%       {m_ens['cov95']:>15.1f}%       "
          f"{m_ens['cov95']-base_cov95:>+9.2f}")
    print(f"  {'NLL':<20} {base_nll:>15.3f}        {m_ens['nll']:>15.3f}        "
          f"{m_ens['nll']-base_nll:>+9.3f}")

    # ⭐ 광양항/예천/고흥만 비교
    print("\n  ⭐ 광양항세방 / 예천 / 고흥만수상 비교:")
    target_sites = ["광양항세방", "예천", "고흥만수상"]
    base_per_site_path = ROOT / "pv/experiments/resmlp_adaln_ensemble/ensemble_per_site.csv"
    if base_per_site_path.exists():
        base_per_site = pd.read_csv(base_per_site_path)
        print(f"\n  {'Site':<14} {'Metric':<8} {'Baseline':<12} {'Site Emb':<12} {'Δ':>8}")
        for s in target_sites:
            b = base_per_site[base_per_site.site == s].iloc[0]
            n = site_ens[site_ens.site == s].iloc[0]
            for metric in ["nmae", "cov80", "cov95"]:
                delta = n[metric] - b[metric]
                print(f"  {s:<14} {metric:<8} {b[metric]:>10.2f}{'':3} {n[metric]:>10.2f}{'':3} "
                      f"{delta:>+8.2f}")

    site_ens.to_csv(OUT_DIR / "ensemble_per_site.csv", index=False)
    seed_df.to_csv(OUT_DIR / "per_seed_summary.csv", index=False)
    print(f"\n저장: {OUT_DIR}")


if __name__ == "__main__":
    main()
