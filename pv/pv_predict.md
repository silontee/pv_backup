...
\section{문제 설정과 표기법}
지역 집합을 $\mathcal R = \{1,\dots,R\}$라 하고, 예시적으로 $\mathcal R=\{\text{Gangwon},\text{Jeju},\text{Gyeonggi}\}$라 둘 수 있다. 지역 $r\in\mathcal R$에 속한 사이트 집합을 $\mathcal S_r$라 하며, 사이트 $i\in\mathcal S_r$의 시점 $t$에서의 관측 발전량을 $y_{i,t}$라 한다. 사이트 $i$의 설치용량을 $C_i$라 두고, 정규화된 출력
\begin{equation}
...
\begin{itemize}[leftmargin=2em]
    ...
    \item \textbf{과거/미래 기상 변수} $\bm x_{i,\tau}$: GHI, DNI, DHI, 기온, 상대습도, 풍속, 전운량, cloud index 등
    \item \textbf{달력/태양기하 변수} $\bm c_{\tau}$: hour, day-of-year, weekday, holiday, solar zenith/azimuth
    \item \textbf{지역 메타데이터} $\bm m_r$: 위도, 경도, 고도, 해안/내륙 여부, 기후권역, 설치용량, tilt, azimuth 등
\end{itemize}
...
\begin{equation}
    ...
    \label{eq:beta_obs}
\end{equation}
여기서 $\tilde y^{\star}_{i,t}$는 $0$과 $1$을 피하기 위해 작은 $\epsilon>0$에 대해
\begin{equation}
    \tilde y^{\star}_{i,t} = \frac{\tilde y_{i,t}(N-1)+0.5}{N}
\end{equation}
또는 $\tilde y^{\star}_{i,t}=\min\{\max(\tilde y_{i,t},\epsilon),1-\epsilon\}$와 같이 변환한 값이다. $\phi_r$는 지역별 정밀도(precision) 파라미터이다.
...
\end{equation}
로 두어 지역마다 주간 출력 분산 수준이 다를 수 있게 한다.
...
\begin{equation}
    ...
    \qquad h=1,\dots,H
\end{equation}
로 정의한다. 실무적으로는 direct multi-horizon 예측을 위해
\begin{equation}
...
\begin{equation}
    ...
    \Big],
\end{equation}
여기서 $\mathrm{Emb}(r)$는 지역 ID embedding, $\bm m_r$는 원시 지역 메타데이터, $\bar{\bm z}_r^{\mathrm{stat}}$는 압축된 베이지안 지역 통계 서명이다.
...
최종 예측값은
\begin{equation}
    \hat{\tilde y}_{i,t+h} = \hat \mu^{\mathrm{Bayes}}_{i,t+h} + \hat e_{i,t+h}
...
\subsection{기본 손실함수}
다시계점 잔차 예측의 기본 손실은 weighted Huber loss 또는 weighted MSE를 권장한다.
\begin{equation}
    \mathcal L_{\mathrm{res}} = \sum_{h=1}^H w_h \, \ell\Big(e_{i,t+h}, \hat e_{i,t+h}\Big),
\end{equation}
여기서 $w_h$는 horizon별 가중치이며 가까운 horizon에 더 큰 값을 둘 수 있다.
...
\begin{align}
    ...
    \mathcal L_{\mathrm{agg}} &= \sum_{r\in\mathcal R}\sum_{h=1}^H \Big(\hat Y_{r,t+h} - \sum_{i\in\mathcal S_r} \hat y_{i,t+h}\Big)^2.
\end{align}
